...
export const CommunityScreen = ({
  ...
  const StatButton = ({ active, icon: Icon, count, onClick, ariaLabel }) => (
    <button
      ...
      className="flex items-center gap-1.5 text-[13px] px-3 py-1.5 rounded-full transition-all active:scale-95"
      style={{
    ...
    return (
      <GlassCard theme={theme} className="p-4 space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p className="font-semibold text-[13px]" style={{ color: theme.colors.textPrimary }}>{post.user?.name}</p>
              <span className="text-[11px] font-medium" style={{ color: theme.colors.textSecondary }}>{post.timeAgo}</span>
            </div>
            {post.title && <p className="font-bold text-[13px] mt-0.5" style={{ color: theme.colors.textPrimary }}>{post.title}</p>}
            {post.text && <p className="text-[13px] mt-1 whitespace-pre-line" style={{ color: theme.colors.textSecondary }}>{post.text}</p>}
          </div>
        </div>
        {post.image && (
        ...
        <div className="flex items-center gap-2 pt-1">
          ...
          <button onClick={()=>{ if(navigator.share){ navigator.share({ title: post.title || 'Post', text: post.text }); } else { navigator.clipboard.writeText(post.text || window.location.href); } }} className="flex items-center gap-1.5 text-[13px] px-3 py-1.5 rounded-full transition-all active:scale-95" style={{ backgroundColor: '#ffffff', border:`1px solid ${theme.colors.border}`, color: theme.colors.textSecondary, boxShadow:'0 1px 2px rgba(0,0,0,0.04)' }}>
            <Share2 className="w-4 h-4" /> <span>Share</span>
          </button>
        </div>
        <div style={{ maxHeight: expandedComments[post.id] ? measuredHeight : 0, opacity: expandedComments[post.id] ? 1 : 0, transition: 'max-height 300ms ease, opacity 250ms ease', overflow: 'hidden' }}>
          <div ref={contentRef} className="pt-2 space-y-3">
            ...
            <form onSubmit={submitComment} className="flex items-center gap-2">
              <input value={draft} onChange={e=>setDraft(e.target.value)} placeholder="Add a comment" className="flex-1 text-[13px] px-3 py-2 rounded-full outline-none" style={{ backgroundColor: '#ffffff', color: theme.colors.textPrimary, border:`1px solid ${theme.colors.border}` }} />
    ...
    return (
      <GlassCard theme={theme} className="p-4 space-y-3">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p className="font-semibold text-[13px]" style={{ color: theme.colors.textPrimary }}>{poll.user?.name}</p>
              <span className="text-[11px] font-medium" style={{ color: theme.colors.textSecondary }}>{poll.timeAgo}</span>
            </div>
            <p className="text-[13px] mt-1 font-medium" style={{ color: theme.colors.textPrimary }}>{poll.question}</p>
              ...
              {poll.options.map(opt => {
                ...
                return (
                  <button key={opt.id} type="button" disabled={!!votedOption} onClick={()=>onPollVote?.(poll.id,opt.id)} className="w-full text-left px-3 py-2 rounded-xl border relative overflow-hidden group whitespace-nowrap" style={{ borderColor: active ? theme.colors.accent : theme.colors.border, backgroundColor: '#ffffff', color: active ? theme.colors.accent : theme.colors.textPrimary }}>
                    <span className="relative z-10 text-[11px] font-medium flex justify-between">
                      <span>{opt.text}</span>
    ...
    <div className="py-3 space-y-2">
      <div className="flex items-center gap-3">
        ...
        <button 
          ...
          className="h-10 ml-auto inline-flex items-center justify-center gap-2 rounded-full text-[13px] font-bold transition-all active:scale-95 shadow-md px-4" 
      ...
      <StandardSearchBar
        value={query}
        onChange={setQuery}
        placeholder={viewMode==='feed'? 'Search posts...':'Search library...'}
      ...
      {effectiveViewMode === 'feed' && (
        <>
          {!filteredContent.length && (
            <div className="text-center text-[13px] pt-20" style={{ color: theme.colors.textSecondary }}>
              No content found.
            </div>
          )}
        ...
        <div className={`grid gap-3 ${isDesktop ? 'grid-cols-3' : 'grid-cols-2'}`}>
          {photoLibrary.map(photo => (
            ...
            >
              <img 
                src={photo.src} 
                alt={photo.post.title||'post image'} 
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/50 via-black/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-2">
                <p className="text-[11px] leading-tight text-white line-clamp-2">
                  {photo.post.title || photo.post.text || photo.post.user?.name}
                </p>
              </div>
            </div>
          ))}
          {!photoLibrary.length && (
            <p className="col-span-full text-center text-[13px]" style={{ color: theme.colors.textSecondary }}>
              No photos found.
            </p>
          )}
        </div>
